...
class AssignmentView(APIView):
    ...
    def get(self, request: Request):
        ...
        try:
            ...
            chosen_proxy.save()
            return Response(data=f"{chosen_proxy.function_url}", status=status.HTTP_200_OK)
...
class ProxyUpdateView(APIView):
    ...
    def post(self, request: Request):
        ...
        for proxy_id in range(proxy_len):
            ...
            proxy_url = base_proxy_url.format(num1, num2, source_id) ## ??
            proxy = Proxy.objects.get(function_url=proxy_url)
            clients = Assignment.objects.filter(proxy=proxy).values_list(
                "client", flat=True
            )
            new_proxy_url = base_proxy_url.format(num1, num2, source_id + 1) ## ??
            new_proxy = Proxy.objects.get(function_url=new_proxy_url)
            ...
            ### MODIFY THIS PART ###
            # lambda doesn't have a ip and can't comm w/ socket
            # This endpoint 
            # migration_socket.connect((proxy_url, 8121))
            # migration_socket.send(f"migrate {new_proxy_url}".encode())
...
class RealProxyUpdateView(APIView):
    ...
    def post(self, request: Request):
        # TODO: Migrating everything for now, fix later maybe!
        old_urls = request.data["old_urls"] # expects a list
        new_urls = request.data["new_urls"] # expects a list
        old_arns = request.data["old_arns"]

        if len(old_urls) == 0: # creation or scale up
            if len(new_urls) == 0: # To scale up, we need some new_ips
                return JsonResponse({'error': 'Invalid or missing new_urls, when old_urls is provided'}, status=400)
            for url in new_urls: # create new proxy in database
                Proxy.objects.create(function_url=url)
        elif len(new_urls) == 0: # scale down
            if len(old_urls) == 0: # To scale down, we need some old_ips
                return JsonResponse({'error': 'Invalid or missing old_urls, when new_urls is provided'}, status=400)
            
            # TODO: reassign clients to existing non-terminated proxies

            for url in old_urls: # remove old proxies from the database
                proxy_to_delete = Proxy.objects.get(function_url=url)
            ...
            try:
                ...
                for i in range(len(old_urls)):
                    proxy_url = old_urls[i]
                    proxy = Proxy.objects.get(function_url=proxy_url)
                    clients = Assignment.objects.filter(proxy=proxy).values_list(
                        "client", flat=True
                    )
                    new_proxy_url = new_urls[i]
                    
                    new_proxy, _ = Proxy.objects.get_or_create(function_url=new_proxy_url)
                    for client_ip in clients:
                    ...
                    # migration_socket.send(f"migrate {new_proxy_url}".encode())
                    # migration_socket.close()
                    region = old_arns[i].split(':')[3]
                    client = boto3.client('lambda', region)
                    # print(old_arns[i])
                    response = client.tag_resource(
                        Resource=old_arns[i], # resource could be an instance, network interface, eip, etc
                        Tags={
                                'new-lambda-url': new_proxy_url
                            }
                    )
        ...
        return Response(
            data=f"Updated assignments from {old_urls} -> {new_urls} and sent migration info!",
            status=status.HTTP_200_OK,
        )
...
class ProxyAvgPostView(APIView):
    ...
    def post(self, request: Request):
        try:
            avg_time = float(request.data["avg"])
            proxy_url = request.META.get("REMOTE_ADDR", None)
        except:
            return Response(data=f"bad request", status=status.HTTP_400_BAD_REQUEST)

        obj = ProxyAvgMigrationTime.objects.create(proxy_url=proxy_url, value=avg_time)
