...
def get_client_proxy_utilization(client, client_assignments, right_now):
    ...
    for assignment in client_assignments:
        if proxy_checker.get(assignment.proxy.function_url, None):
            continue
        ...
        proxy_checker[assignment.proxy.function_url] = True
...
def request_new_proxy(proposing_clients, right_now: int):
    ...
    for proxy in active_proxies:
        ...
        proxy_prefrences[proxy.function_url] = list(
            reversed(
                sorted(
                    utility_values_for_clients,
        ...
        proxy_capacities[proxy.function_url] = proxy.capacity
    ...
    for proxy in active_proxies:
        ...
        general_proxy_utilities[proxy.function_url] = proxy_utility
    ...
    for client in proposing_clients:
        ...
        for proxy in active_proxies:
            ...
            proxy_utility = general_proxy_utilities[proxy.function_url] - beta4 * distance
            utility_values_for_proxies[proxy.function_url] = proxy_utility
    ...
    for proxy_id in matches.keys():
        proxy = Proxy.objects.get(function_url=proxy_id)
