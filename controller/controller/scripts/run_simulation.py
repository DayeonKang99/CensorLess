...
def run_simulation(
    ...
    last_created_proxy_url = "https://tdmkiocbt3xykrle7thpqqnbgy0narzy.lambda-url.us-west-1.on.aws/"
    last_created_client_id = -1
    Proxy.objects.create(function_url=last_created_proxy_url, is_test=True)
    ...
    for step in tqdm(range(duration)):
        ...
        if step % NEW_PROXY_INTERVAL == 0:
            for _ in range(NEW_PROXY_COUNT):
                last_created_proxy_url = create_new_proxy(last_created_proxy_url)
...
def rejuvinate(step):
    ...
    for proxy in active_proxies:
        old_proxy_url = proxy.function_url
        new_proxy_url = get_migration_proxies_url(old_proxy_url)
        # new_proxy = Proxy.objects.create(ip=new_proxy_ip, is_test=True)
        proxy.function_url = new_proxy_url
        if proxy.is_blocked:
...
def url_generator(size=31, chars=string.ascii_uppercase+string.ascii_lowercase+string.digits):
    return ''.join(random.choice(chars) for _ in range(size))

def get_migration_proxies_url(old_url):
    domain = old_url.split("://")
    h = domain[1].split(".")
    url = "https://"+url_generator()+'.'+'.'.join(h[1:])
...
def create_new_proxy(last_created_proxy_url):
    domain = last_created_proxy_url.split("://")
    h = domain[1].split(".")
    last_created_proxy_url = "https://"+url_generator()+'.'+'.'.join(h[1:])
    ...
    # last_created_proxy_url = f"{nums[0]}.{nums[1]}.{nums[2]}.{nums[3]}"
    Proxy.objects.create(function_url=last_created_proxy_url, is_test=True)
    return last_created_proxy_url
...
def create_new_client(
    ...
    if random.random() < censor_chance:
